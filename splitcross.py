...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, hidden_size, splits, verbose=False):
        ...
        # For example, a vocab of 1000 words may have splits [0] + [100, 500] +
        # [inf]
        ...
        # Each of the splits that aren't in the head require a pretend token,
        # we'll call them tombstones
        # The probability given to this tombstone is the probability of
        # selecting an item from the represented split
        if self.nsplits > 1:
            self.tail_vectors = nn.Parameter(
                torch.zeros(self.nsplits - 1, hidden_size))
    ...
    def logprob(self, weight, bias, hiddens, splits=None,
        ...
        # First we perform the first softmax on the head vocabulary and the
        # tombstones
        if softmaxed_head_res is None:
            ...
            if self.nsplits > 1:
                head_weight = (self.tail_vectors
                               if head_weight is None
                               else torch.cat([head_weight,
                                               self.tail_vectors]))
                head_bias = (self.tail_bias
                             if head_bias is None
                             else torch.cat([head_bias, self.tail_bias]))

            # Perform the softmax calculation for the word vectors in the head
            # for all splits
        ...
        for idx in splits:
            ...
            # For those targets in the head (idx == 0) we only need to return
            # their loss
            if idx == 0:
                results.append(softmaxed_head_res[:, :-(self.nsplits - 1)])

            # If the target is in one of the splits, the probability is the
            # p(tombstone) * p(word within tombstone)
            else:
    ...
    def split_on_targets(self, hiddens, targets):
        ...
        # Determine to which split each element belongs (for each start split
        # value, add 1 if equal or greater)
        # This method appears slower at least for WT-103 values for approx
        # softmax
        # masks = [(targets >= self.splits[idx]).view(1, -1) for idx in
        # range(1, self.nsplits)]
        # mask = torch.sum(torch.cat(masks, dim=0), dim=0)
        ###
        # This is equally fast for smaller splits as method below but scales
        # linearly
        mask = None
        ...
        for idx in range(self.nsplits):
            ...
            if self.nsplits == 1:
                ...
                continue
            # If all the words are covered by earlier targets, we have empties
            # so later stages don't freak out
            ...
            split_hiddens.append(hiddens
                                 .masked_select(tmp_mask
                                                .unsqueeze(1)
                                                .expand_as(hiddens))
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        if self.verbose or verbose:
            for idx in sorted(self.stats):
                print('{}: {}'.format(
                    idx, int(np.mean(self.stats[idx]))), end=', ')
            print()
        ...
        # Perform the softmax calculation for the word vectors in the head for
        #  all splits
        # We need to guard against empty splits as torch.cat does not like
        # random lists
        combo = torch.cat([split_hiddens[i]
        ...
        for idx in range(self.nsplits):
            ...
            if idx == 0:
                ...
                entropy = -torch.gather(softmaxed_head_res,
                                        dim=1,
                                        index=split_targets[idx].view(-1, 1))
            # If the target is in one of the splits, the probability is the
            # p(tombstone) * p(word within tombstone)
            else:
                softmaxed_head_res = softmaxed_all_head_res[
                ...
                if self.verbose or verbose:
                    start, end = self.splits[idx], self.splits[idx + 1]
                    tail_weight = weight[start:end]
                    self.stats[idx].append(split_hiddens[idx].size()[
                                           0] * tail_weight.size()[0])
                ...
                # All indices are shifted - if the first split handles
                # [0,...,499] then the 500th in the second split will be 0
                # indexed
            ...
            total_loss = (entropy.float().sum()
                          if total_loss is None else total_loss +
                          entropy.float().sum())
